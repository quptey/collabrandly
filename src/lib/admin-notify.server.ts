...
export const submitCreatorApplication = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    await supabaseAdmin.from("creator_info").upsert({
      ...
      other_social_links: appData.socialPlatform && !['Instagram', 'TikTok', 'YouTube'].includes(appData.socialPlatform)
        ? JSON.parse(`[{"platform":"${appData.socialPlatform}","url":"${appData.profileLink}"}]`)
        : '[]',
      bio: '',
    ...
    const html = `
      ...
      <p style="margin-top:20px;color:#888;font-size:12px;">creator·kz · Admin Notification</p>
    `;

    const { error } = await resend.emails.send({
      from: "creator·kz <notifications@axenuurexe.resend.app>",
      to: ADMIN_EMAIL,
