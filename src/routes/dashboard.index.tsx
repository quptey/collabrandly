...
export const Route = createFileRoute("/dashboard/")({
  head: () => ({ meta: [{ title: "Dashboard — Collabrandly" }] }),
  component: Dashboard,
  errorComponent: DashboardErrorBoundary,
});

function Dashboard() {
  const { user, loading, role, isShopper, isCreator, isBrand, isAdmin, isPending, isApproved } =
    useAuth();
  ...
  async function setStatus(id: string, status: "accepted" | "rejected") {
    ...
    toast.success(
      status === "accepted" ? t("toasts.requestAccepted") : t("toasts.requestRejected"),
    );
  ...
  if (!profile.onboarded && profile.role !== "shopper") {
    ...
    <div className="min-h-screen bg-background">
      ...
      <ErrorBoundary>
        <div className="mx-auto max-w-6xl px-6 py-12">
          ...
          {isCreator && (
            <div className="mb-10 grid gap-3 sm:gap-4 grid-cols-2 sm:grid-cols-2 lg:grid-cols-4">
              ...
              <div className="rounded-2xl border border-border bg-card p-5">
                <p className="text-[10px] uppercase tracking-wider text-muted-foreground">
                  {t("dashboard.collaborationRequests")}
                </p>
                ...
                <div className="mt-2">
                  ...
                  <div className="mt-1.5 h-2 overflow-hidden rounded-full bg-secondary">
                    <div
                      ...
                      style={{
                        width: `${Math.min(100, [profile.display_name, profile.bio, profile.avatar_url, profile.category, profile.city, profile.social_link].filter(Boolean).length * 17)}%`,
                      }}
                    />
                  </div>
                </div>
          ...
          {isCreator && !isPending && !isApproved && (
            <div className="mb-8 rounded-2xl border border-accent/20 bg-accent/5 p-6">
              <p className="font-display text-xl font-semibold">
                {t("dashboard.submitForVerification")}
              </p>
              <p className="mt-2 text-sm text-muted-foreground">
          ...
          {isCreator && (
            <div className="mb-8 grid gap-3 sm:gap-4 grid-cols-2 sm:grid-cols-2 lg:grid-cols-4">
              <Link
                to="/profile"
              ...
              >
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {t("dashboard.browseMarketplaceDesc")}
                </p>
              </Link>
              <Link
                to="/dashboard"
              ...
              >
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {t("dashboard.viewPublicProfileDesc")}
                </p>
              </Link>
            </div>
          )}
          ...
          <Tabs
            defaultValue={isCreator || isShopper ? "collections" : "requests"}
            className="space-y-6 sm:space-y-8"
          >
            <TabsList className="bg-secondary/40 w-full overflow-x-auto [&::-webkit-scrollbar]:hidden">
              {(isCreator || isShopper) && (
            ...
            {isCreator && (
              <TabsContent value="collections" className="space-y-6">
                <div className="flex items-center justify-between">
                  ...
                  <Button variant="accent" onClick={() => setNewColOpen(true)}>
                ...
                {collections.length === 0 ? (
                  <div className="rounded-3xl border border-dashed border-border bg-card p-16 text-center">
                    ...
                    <Button variant="accent" onClick={() => setNewColOpen(true)} className="mt-6">
                        ...
                        >
                          <div className="aspect-[16/10] overflow-hidden bg-warm">
                            {c.cover_url ? (
                              ...
                              <div className="grid h-full place-items-center bg-gradient-to-br from-warm to-cream font-display text-2xl text-foreground/30">
                                {c.title}
                              </div>
                            )}
                          </div>
                          <div className="space-y-3 p-5">
                            <div>
                              ...
                              <p className="mt-1 text-xs text-muted-foreground">
                                {count}{" "}
                                {count === 1 ? t("dashboard.product") : t("dashboard.products")}
                              </p>
                            </div>
              ...
              {requests.length === 0 ? (
                ...
                <div className="space-y-3">
                  {requests.map((r) => {
                    let goalMeta: any = {};
                    try {
                      if (r.goal) goalMeta = JSON.parse(r.goal);
                    } catch {
                      goalMeta = {};
                    }
                    return (
                      ...
                      >
                        <div className="flex flex-wrap items-start justify-between gap-3">
                          <div className="flex items-start gap-4">
                            ...
                            <div>
                              <p className="font-display text-xl font-semibold">
                                {goalMeta.campaignName || "Collaboration Request"}
                              </p>
                              <p className="text-sm text-muted-foreground">
                                from {r.profiles?.display_name || r.brand_name}
                              </p>
                            </div>
                          </div>
            ...
            <TabsContent value="notifications" className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                {notifications.some((n) => !n.read_at) && (
                  <Button variant="outline" size="sm" onClick={markAllNotifRead}>
              ...
              {notifications.length === 0 ? (
                ...
                <div className="space-y-2">
                  {notifications.map((n) => (
                    <div
                      key={n.id}
                      className={`group relative rounded-2xl border border-border bg-card p-4 sm:p-5 cursor-pointer transition-colors hover:bg-secondary/20 ${n.read_at ? "opacity-60" : ""}`}
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle className="font-display text-2xl">
                {t("dashboard.newCollectionDialog")}
              </DialogTitle>
            </DialogHeader>
            <form onSubmit={colHandle(createCollection)} className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <Input
                  ...
                  onChange={(v) => colSet("cover_url", v.target.value)}
                  placeholder="https://…"
                />
              </div>
              <Button type="submit" variant="accent" className="w-full" disabled={colSub}>
                {colSub ? t("dashboard.creating") : t("dashboard.create")}
              </Button>
            </form>
          </DialogContent>
        </Dialog>
  ...
  const { data: featured = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase
        .from("profiles")
        .select(
          "id, display_name, avatar_url, bio, category, custom_category, city, follower_range",
        )
        .eq("role", "creator")
          ...
          <Link key={c.id} to="/creator/$id" params={{ id: c.id }}>
            <div className="overflow-hidden rounded-2xl border border-border bg-card transition-colors hover:border-accent/50">
              ...
              <div className="p-4">
                ...
                <p className="mt-1 text-xs text-muted-foreground">
                  {c.category && getCategoryLabel(t, c.category, c.custom_category)}
                  {c.city ? ` · ${c.city}` : ""}
                </p>
              </div>
            </div>
...
function ProfileEditor({ profile, onSaved }: { profile: any; onSaved: () => void }) {
  ...
  const form = useForm({
    ...
    values: {
      ...
      follower_count: (profile as any).follower_count ?? 0,
  ...
  async function save(data: any) {
    ...
    if (isCreator) {
      ...
      (payload as any).follower_count = data.follower_count ? Number(data.follower_count) : null;
    }
  ...
  return (
    <form
      ...
      className="max-w-2xl space-y-6 rounded-3xl border border-border bg-card p-8 pb-24 sm:pb-8"
    >
      <h2 className="font-display text-2xl font-semibold">{t("dashboard.editProfileTitle")}</h2>
      <div className="space-y-1.5">
        <Label htmlFor="pn">
          {isCreator ? t("dashboard.displayName") : t("dashboard.brandName")}
        </Label>
      ...
      {isCreator && (
        <>
          ...
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
            ...
            <div className="space-y-1.5">
              <Label>{t("dashboard.followersCount")}</Label>
              <Input
                type="number"
                min="0"
                step="1"
      ...
      <div className="max-md:sticky max-md:bottom-0 max-md:bg-card max-md:pt-4 max-md:border-t max-md:border-border">
