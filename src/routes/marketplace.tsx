  ...
  return (
    <div className="min-h-screen bg-background pb-24 sm:pb-0">
      ...
      <section className="mx-auto max-w-7xl px-6 py-14">
        ...
        {isLoading ? (
          <div className="editorial-grid grid grid-cols-1 gap-3 sm:gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
            {Array.from({ length: 8 }).map((_, i) => (
              <div key={i} className="aspect-[4/5] animate-pulse rounded-3xl bg-warm" />
            ))}
          </div>
        ) : filtered.length === 0 ? (
          <div className="rounded-3xl border border-dashed border-border bg-card p-8 sm:p-16 text-center">
              ...
              return (
                <div key={c.id} className="group relative">
                  ...
                  <button
                    ...
                    className={`absolute right-3 top-3 z-10 grid h-10 w-10 sm:h-8 sm:w-8 place-items-center rounded-full border bg-background/80 backdrop-blur-sm transition-colors ${isSaved ? "border-accent text-accent" : "border-border text-muted-foreground hover:text-foreground"}`}
                  >
