    ...
    <>
      <div className="aspect-[9/19.5] overflow-hidden rounded-2xl bg-warm">
        {imageUrl ? (
          ...
          <div className="grid h-full w-full place-items-center text-xs text-muted-foreground">{t("creatorProfile.noImage")}</div>
        )}
      </div>
      <div className="mt-3 flex items-start justify-between gap-2">
        <p className="text-sm font-medium leading-snug">{name}</p>
        {externalLink && <ExternalLink className="h-3.5 w-3.5 shrink-0 text-muted-foreground transition-colors group-hover:text-accent" />}
      </div>
    </>
  );
