...
export function CreatorCard({ id, name, avatar, bio, category, city, followerRange }: CreatorCardProps) {
  const initials = name
    .split(" ")
    .map((n) => n[0])
    .slice(0, 2)
  ...
  return (
    ...
    >
      <div className="aspect-[4/5] overflow-hidden bg-warm">
        {avatar ? (
          <img src={avatar} alt={name} className="h-full w-full object-cover transition-transform duration-700 group-hover:scale-105" />
        ) : (
          <div className="grid h-full w-full place-items-center bg-gradient-to-br from-warm to-cream">
            <span className="font-display text-6xl font-semibold text-foreground/30">{initials || "·"}</span>
          </div>
        )}
      </div>
      <div className="space-y-3 p-5">
        <div className="flex items-start justify-between gap-3">
          <h3 className="font-display text-xl font-semibold leading-tight tracking-tight">{name}</h3>
          {followerRange && (
        ...
        <div className="flex items-center gap-2 pt-1 text-xs uppercase tracking-wider text-muted-foreground">
          {category && <span className="font-medium text-accent">{CATEGORY_LABELS[category]}</span>}
